...
public class SendMailUtil {
    ...
    // 收件人邮箱（替换为自己知道的有效邮箱）
    /*public static String receiveMailAccount = "crdchen@foxmail.com";
    ...
    public static void main(String[] args) throws Exception {
        String receiveMailAccount = "crdchen@foxmail.com";
        String sendName = "Marsdl火星探索实验室";
        String receiveName = "钱大刚";
        ...
        // 3. 创建一封邮件
        MimeMessage message = null;/*createMimeMessage(session, myEmailAccount, receiveMailAccount);*/
     ...
     * @param receiveName
     */
    public static void sendEmail(String receiveMailAccount, String receiveName) {
        Properties props = new Properties();                    // 参数配置
        props.setProperty("mail.transport.protocol", "smtp");   // 使用的协议（JavaMail规范要求）
        props.setProperty("mail.smtp.host", myEmailSMTPHost);   // 发件人的邮箱的 SMTP 服务器地址
        props.setProperty("mail.smtp.auth", "true");            // 需要请求认证
        ...
        try {
            message = createMimeMessage(session, myEmailAccount, receiveMailAccount, receiveName);
            // 4. 根据 Session 获取邮件传输对象
            Transport transport = session.getTransport();
            // 5. 使用 邮箱账号 和 密码 连接邮件服务器, 这里认证的邮箱必须与 message 中的发件人邮箱一致, 否则报错
            //
     ...
     * @throws Exception
     */
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail, String receiveName) throws Exception {
        // 1. 创建一封邮件
